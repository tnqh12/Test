...
public class TransactionExer2 {
	...
	public static void main(String[] args) {
		...
		BankAccont hong = new BankAccont(1,"홍길동",1000);
		BankAccont kang = new BankAccont(2,"강감찬",0);
		
		Savepoint beforeWithdraw;
	...
	//출금
	private static int withdraw(BankAccont ba, int money) throws SQLException {
		//계좌 잔액이 출금 금액 이상인지
	...
	//입금
	private static int deposit(BankAccont ba, int money) throws SQLException{
		String sql = " update bankaccount set bamoney=? where baid=? ";
		PreparedStatement ps = ConnectionUtil.getConnection().prepareStatement(sql);
		ps.setInt(1, ba.getBamoney()+ money);
		ps.setInt(2, ba.getBaid());
